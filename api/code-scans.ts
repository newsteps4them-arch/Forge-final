...
async function createFixPullRequest(ctx: GitHubContext, files: Map<string, string>, findings: Finding[]): Promise<string> {
  ...
  const newCommit = await github<{ sha: string }>(ctx, `/repos/${ctx.owner}/${ctx.repo}/git/commits`, {
    method: "POST",
    body: JSON.stringify({
      message: "chore: apply automated code scan fixes",
      tree: newTree.sha,
      parents: [base.commit.sha],
    }),
  });

  await github(ctx, `/repos/${ctx.owner}/${ctx.repo}/git/refs/heads/${branchName}`, {
    method: "PATCH",
    body: JSON.stringify({
      sha: newCommit.sha,
    }),
