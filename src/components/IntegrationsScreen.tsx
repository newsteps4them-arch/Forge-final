...
export const IntegrationsScreen = ({ onBack, connectedIds, onToggleConnection }: { 
  ...
  const handleSync = (id: string, name: string) => {
    ...
    setTimeout(() => {
       setSyncingId(null);
       toast.show(`Sync complete. Data from ${name} integrated into Neural_Sync.`, "success");
    }, 2500);
  };
    ...
    >
      <div className="flex items-center gap-3 px-6 mb-4">
        ...
        <div className="flex items-center gap-2 text-primary">
          ...
          <h2 className="text-xl font-black uppercase tracking-widest">3rd Party APIs</h2>
        </div>
      </div>
      ...
      <div className="px-6 space-y-6 overflow-y-auto no-scrollbar flex-1 pb-10 mt-2">
         <div className="bg-primary/5 border border-primary/20 rounded-3xl p-5 mb-2">
            ...
            <p className="text-[10px] text-white/60 font-mono leading-relaxed uppercase tracking-wider">
               Connect external proprietary software to automatically ingest logs, telemetry, and repair databases into the Forge Neural engine.
            </p>
         </div>
         ...
         <div className="space-y-4">
            {INTERGRATIONS.map(int => {
              ...
              return (
                <div key={int.id} className={`p-4 rounded-[2rem] border transition-all ${isConnected ? 'bg-primary/5 border-primary/30' : 'bg-card/40 border-white/5'} overflow-hidden relative`}>
                   
                   <div className="flex items-start justify-between relative z-10">
                      <div className="flex items-center gap-4">
                         <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${isConnected ? 'bg-primary text-black' : 'bg-white/5 text-primary/40'}`}>
                            <int.icon className="w-6 h-6" />
                         </div>
                         <div className="flex flex-col">
                            <h4 className="text-[15px] font-bold text-white tracking-tight">{int.name}</h4>
                            <span className="text-[9px] text-primary/60 font-black uppercase tracking-widest leading-none mt-1">{int.type}</span>
                            <span className="text-[10px] text-text-dim mt-1.5 font-mono opacity-80">{int.desc}</span>
                         </div>
                      </div>
                   </div>
         ...
         <div className="mt-8 flex flex-col items-center justify-center opacity-30 text-center gap-2">
            ...
            <span className="text-[9px] font-mono uppercase tracking-widest text-text-dim">Some integrations require API keys stored in user settings. Data is processed locally within the device context before cloud sync.</span>
         </div>
      </div>
    </motion.div>
  );
