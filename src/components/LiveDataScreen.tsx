    ...
    >
      <div className="flex items-center gap-3 px-6 mb-4">
        ...
        <div className="flex items-center gap-2 text-primary">
          ...
          <h2 className="text-xl font-black uppercase tracking-widest">Tuner Hub</h2>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 space-y-6 no-scrollbar pb-10">
        ...
        {tab === 'maps' && (
          <motion.div key="maps" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="space-y-4">
             <div className="bg-primary/10 border border-primary/20 rounded-2xl p-4 flex gap-3">
                <Zap className="w-5 h-5 text-primary flex-shrink-0" />
                <div>
                   <p className="text-[10px] uppercase tracking-widest font-bold text-primary mb-1">Tuning Mode Active</p>
                   <p className="text-xs text-primary/70 leading-relaxed">
                     Modifying 3D volumetric efficiency maps. Ensure ECU is unlocked before writing flash data.
                   </p>
                </div>
             </div>
             
             <div className="text-[10px] font-black uppercase tracking-widest text-white/40 border-b border-white/5 pb-2">Target Maps</div>
             {[
               { id: 'Fuel_Base', name: 'Open Loop Fueling Map', load: '10x10' },
               { id: 'Spark_Adv', name: 'Ignition Timing Advance', load: '12x12' },
               { id: 'Boost_Tgt', name: 'Wastegate Duty Cycle', load: '8x8' },
             ].map(map => (
               <div key={map.id} className="bg-white/5 border border-white/10 rounded-2xl p-4 hover:border-primary/30 transition-colors cursor-pointer group">
                  <div className="flex justify-between items-start mb-2">
                     <span className="text-[10px] font-mono text-primary/60 uppercase tracking-widest">{map.id}</span>
                     <span className="text-[9px] min-w-12 text-center py-0.5 bg-white/10 text-white rounded font-mono shadow-inner">{map.load}</span>
                  </div>
                  <h4 className="text-sm font-bold text-white mb-2">{map.name}</h4>
                  
                  {/* Fake Grid Map */}
                  <div className="grid grid-cols-4 gap-1 opacity-50 group-hover:opacity-100 transition-opacity">
                     {Array.from({length: 8}).map((_, i) => (
                       <div key={i} className={`h-4 rounded-sm ${i % 3 === 0 ? 'bg-primary/40' : (i % 2 === 0 ? 'bg-orange-500/40' : 'bg-red-500/40')}`} />
                     ))}
                  </div>
               </div>
             ))}
