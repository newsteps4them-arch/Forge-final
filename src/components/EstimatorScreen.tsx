...
import { ArrowLeft, Calculator, FileText, Send, Plus, Trash2, Database, Search } from 'lucide-react';
import { toast } from '../lib/notifications';

export const EstimatorScreen = ({ onBack, vehicle }: { onBack: () => void, vehicle: string }) => {
  const [items, setItems] = useState([
    { id: 1, type: 'Labor', desc: 'Engine Diagnostic / Scan', qty: 1, rate: 145.00 },
    { id: 2, type: 'Parts', desc: 'OEM Ignition Coil (P/N 12345)', qty: 1, rate: 89.99 },
    { id: 3, type: 'Labor', desc: 'Ignition Coil Replacement', qty: 0.5, rate: 145.00 },
  ]);
  const [isQuerying, setIsQuerying] = useState(false);

  const laborTotal = items.filter(i => i.type === 'Labor').reduce((acc, curr) => acc + (curr.qty * curr.rate), 0);
  const partsTotal = items.filter(i => i.type === 'Parts').reduce((acc, curr) => acc + (curr.qty * curr.rate), 0);
  const total = laborTotal + partsTotal;

  const handleSend = () => {
     toast.show("Estimate PDF generated and sent to customer via SMS.", "success", 4000);
  };
  ...
  const handlePartsQuery = () => {
     ...
     setTimeout(() => {
        setIsQuerying(false);
        setItems([...items, { id: Date.now(), type: 'Parts', desc: 'NGK Spark Plug Set (Auto-Quoted)', qty: 4, rate: 12.50 }]);
    ...
    >
      <div className="flex items-center justify-between px-6 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 text-primary">
            ...
            <h2 className="text-xl font-black uppercase tracking-widest">Estimator</h2>
          </div>
        </div>
      ...
      <div className="px-6 mb-4">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex justify-between items-center">
           <div>
             <div className="text-[10px] text-white/40 uppercase font-bold tracking-widest mb-1">Target Vehicle</div>
      ...
      <div className="flex-1 overflow-y-auto px-6 space-y-3 no-scrollbar pb-10">
        {items.map(item => (
          <div key={item.id} className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col gap-2 relative group hover:border-primary/30 transition-colors">
             <div className="flex justify-between items-start">
               <div className="flex gap-2 items-center">
                 <span className={`text-[9px] uppercase font-black tracking-widest px-2 py-0.5 rounded ${item.type === 'Labor' ? 'bg-blue-500/20 text-blue-500' : 'bg-emerald-500/20 text-emerald-500'}`}>{item.type}</span>
               </div>
               <span className="text-sm font-black text-white">${(item.qty * item.rate).toFixed(2)}</span>
             </div>
             <p className="text-sm text-white/80 font-medium">{item.desc}</p>
             <div className="text-[10px] text-white/40 font-mono">
               {item.qty} {item.type === 'Labor' ? 'hrs' : 'qty'} @ ${item.rate.toFixed(2)}
             </div>
             
             <button onClick={() => toast.show("Edit not supported in preview", "info")} className="absolute bottom-4 right-4 text-white/20 hover:text-red-500 transition-colors">
