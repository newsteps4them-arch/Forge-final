...
const MODULES = [
  ...
  {
    id: "APIM",
    name: "Accessory Protocol Interface",
    bus: "I-CAN",
    status: "offline",
    dtcs: 0,
  },
      ...
      <div className="px-6 mb-6">
        <div className="bg-primary/10 border border-primary/20 rounded-2xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[10px] text-primary/70 uppercase tracking-widest font-bold mb-0.5">
                Gateway Status
              </div>
          ...
          <button
            onClick={() =>
              toast.show("Network-wide proxy alignment initiated...", "info")
            }
              ...
              {MODULES.filter((m) => m.bus === bus).map((mod) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span
                      className={`text-[12px] font-black uppercase tracking-widest ${mod.status === "offline" ? "text-red-500" : mod.dtcs > 0 ? "text-orange-500" : "text-white"}`}
                    >
                      {mod.id}
                    </span>
                    {pinging === mod.id ? (
                  ...
                  <p className="text-[9px] text-white/40 font-mono tracking-wider leading-tight h-6">
                    {mod.name}
                  </p>
                  {mod.dtcs > 0 && (
                    <span className="text-[8px] bg-orange-500/20 text-orange-500 px-1.5 py-0.5 rounded font-mono uppercase mt-2 inline-block shadow-[0_0_8px_rgba(249,115,22,0.3)]">
                      {mod.dtcs} DTC
                    </span>
                  )}
                </div>
